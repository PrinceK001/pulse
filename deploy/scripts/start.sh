...
docker run -d \
    ...
    -p 3307:3306 \
    -e MYSQL_ROOT_PASSWORD=pulse_root_password \
    -e MYSQL_DATABASE=pulse_db \
    -e MYSQL_USER=pulse_user \
    -e MYSQL_PASSWORD=pulse_password \
    -v "${VOLUME_MYSQL}:/var/lib/mysql" \
    -v "${DEPLOY_DIR}/db/mysql-init.sql:/docker-entrypoint-initdb.d/init.sql:ro" \
    --health-cmd 'mysqladmin ping -h localhost -u root -ppulse_root_password' \
    --health-interval 10s \
    --health-timeout 5s \
    --health-retries 5 \
...
docker run -d \
    ...
    -e CLICKHOUSE_USER=pulse_user \
    -e CLICKHOUSE_PASSWORD=pulse_password \
...
docker run -d \
    ...
    --health-cmd '/otelcol-contrib validate --config=/etc/otel-collector.yaml' \
    --health-interval 10s \
    --health-timeout 5s \
    --health-retries 3 \
...
docker run -d \
    ...
    -e MYSQL_USER=pulse_user \
    -e MYSQL_PASSWORD=pulse_password \
    -e MYSQL_WRITER_MAX_POOL_SIZE=10 \
    -e MYSQL_READER_MAX_POOL_SIZE=10 \
    \
    ...
    \
    -e "CLICKHOUSE_R2DBC_URL=r2dbc:clickhouse:http://clickhouse:8123/otel" \
    -e CLICKHOUSE_USERNAME=pulse_user \
    -e CLICKHOUSE_PASSWORD=pulse_password \
