...
public class AthenaJobQueries {
  ...
  public static final String UPDATE_JOB_WITH_EXECUTION_ID = 
      "UPDATE athena_job SET query_execution_id = ?, status = ?, created_at = ?, updated_at = ? " +
      "WHERE job_id = ? AND project_id = ?";

  public static final String UPDATE_JOB_STATUS = 
      "UPDATE athena_job SET status = ?, updated_at = ? " +
      "WHERE job_id = ? AND project_id = ?";

  public static final String UPDATE_JOB_COMPLETED = 
      "UPDATE athena_job SET status = 'COMPLETED', result_location = ?, completed_at = ?, updated_at = ? " +
      "WHERE job_id = ? AND project_id = ?";

  public static final String UPDATE_JOB_STATISTICS = 
      "UPDATE athena_job SET data_scanned_in_bytes = ?, execution_time_millis = ?, " +
      "engine_execution_time_millis = ?, query_queue_time_millis = ?, updated_at = ? " +
      "WHERE job_id = ? AND project_id = ?";

  public static final String UPDATE_JOB_FAILED = 
      "UPDATE athena_job SET status = 'FAILED', error_message = ?, completed_at = ?, updated_at = ? " +
      "WHERE job_id = ? AND project_id = ?";
  ...
  public static final String GET_JOB_BY_ID = 
      ...
      "FROM athena_job WHERE job_id = ? AND project_id = ?";
